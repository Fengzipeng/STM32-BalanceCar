...
//需要发送的数据放到Send_Data_Buf【】数组中；0—9对应1—10通道
//上位机数据传送使用方法
//Channel_Sum:通道总数
//USART_Num：串口号
void Send(u8 Channel_Sum,u8 USART_Num)
{
	...
	{
	 DataScope_Get_Channel_Data(Send_Data_Buf[i],i+1);
	}
	count=DataScope_Data_Generate(Channel_Sum);
	switch (USART_Num)
	{
		case 1:	
	  for(i=0;i<count;i++)
	 ...
	 case 2:	
		 for(i=0;i<count;i++)
	  {
		 USART_SendData(USART2,DataScope_OutPut_Buffer[i]);
