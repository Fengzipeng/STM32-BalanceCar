...
//lcd resolution 
#if LCD_DIRECT==1    //1竖屏 0横屏
	#define  LCD_ROW_NUM    320                //行数
	#define  LCD_COL_NUM    240                //列数
#else
	#define  LCD_ROW_NUM    240                //行数
	#define  LCD_COL_NUM    320                //列数
#endif
...
void LCD_DisALine(u16 x0, u16 y0, u8 dir, u16 lenth, u16 color)
{
		u16 x1,y1;      //dir 0 x->direction 1 y->direction
		x1 = x0;
		y1 = y0;
		if(dir == 1)
			y1 = y1 + lenth;
...
-------------------------------------------------------------------------------------------------------*/
//屏幕分辨率
#define LCD_ROWNUM 320 //行数
#define LCD_COLUMNNUM 240//列数
