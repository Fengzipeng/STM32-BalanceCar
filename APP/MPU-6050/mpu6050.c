...
void MPU6050_Init() //初始化
{
  ...
  MPU6050_Write_Data(PWR_MGMT_1, 0x00);
	MPU6050_Write_Data(SMPLRT_DIV, 0x04);
...
s16 MPU6050_Read_Data(u8 addr) //读取数据
{
	s16 temp;
	IIC_Start();
	IIC_Send_Byte(SlaveAddress);
...
{
	u8 H=0,L=0;
	H=MPU6050_Read_Data(addr);
	L=MPU6050_Read_Data(addr+1);
	return (H<<8)+L;
}
	...
	if(!mpu_init())
  {
	  ...
	  if(!mpu_set_dmp_state(1))
		...
		if(!dmp_set_interrupt_mode(DMP_INT_CONTINUOUS))
			printf("mpu_dmp_int_set compete......\n");
		return 1;
  }
	return 0;
}
